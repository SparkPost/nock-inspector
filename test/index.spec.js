...
describe('nock inspector', function() {
  ...
  it('should require a request and response in respondToRequest()', function() {
    expect(() => postInspector.respondToRequest({ body: { hansel: 'gretal' } })).to.throw(
      'both a request and a response are required'
    );
  });

  it('should require request body or headers in respondToRequest()', function() {
    expect(() => postInspector.respondToRequest({}, { status: 202, body: { hag: 'gingerbread house' } })).to.throw(
      'request must have a body or headers'
    );
  });
});
